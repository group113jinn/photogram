...
export function PostPreview({ post, showModal, onLikePost, onCommentInput, onSaveComment, onToggleComments }) {
  ...
  const isCurrUserLiked = post.reactions.some(reaction => reaction.by.username === "eugene_b") //change to a current logged in user
...
const disablePostButton = () =>{
  ...
  return (
    <ul className="post-preiew-card">
      ...
      <form onSubmit={(ev) => onSaveComment(ev,disablePostButton)}>
        <li className="write-comment"><input name="txt" type="text"  onChange={(ev) => onCommentInput(ev, post,enablePostButton)} placeholder="Write a comment..." ></input><button type="submit" ref={buttonRef} disabled>Post</button></li>
      </form>
