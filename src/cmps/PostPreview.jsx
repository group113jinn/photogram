...
export function PostPreview({ post, showModal, onLikePost}) {
  ...
  <span> {post.reactions.length}{post.reactions.length === 1 ? " like" : " likes"}</span>
        ...
        <li>
          <ul className="comments-box">
            {userComments}
            {/* <li>
  <span>{post.comments.length !== 0?post.comments[0].by.username:""}</span> {post.comments.length !== 0?post.comments[0].txt:""}
            </li> */}
          </ul>
        </li>
      </ul>

      <li className="write-comment"><input name="txt" type="text" placeholder="Write a comment..."></input></li>
    </ul>
  )
}
